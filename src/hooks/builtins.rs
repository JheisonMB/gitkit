...
pub(crate) const ALL: &[Builtin] = &[
    ...
    Builtin {
        ...
        hook: "commit-msg",
        description:
            "Validates commit messages against user-defined regex rules in .gitmessage-rules.json",
        script: MESSAGE_RULES_PLACEHOLDER,
    },
];
...
/// The four message prefixes git writes itself, never a human: `git revert`,
...
/// them. This is the single source of truth — the shell macro below and the
/// Rust evaluation in `message_rules` both derive from this list, so the set
/// can only drift if someone edits this constant without updating its users.
pub(crate) const AUTO_GENERATED_MESSAGE_PREFIXES: &[&str] =
    &["Revert \"", "Merge ", "fixup! ", "squash! "];

/// Returns true if `subject` starts with one of the auto-generated prefixes.
pub(crate) fn is_auto_generated_message(subject: &str) -> bool {
    AUTO_GENERATED_MESSAGE_PREFIXES
        .iter()
...
/// Placeholder script for the `message-rules` builtin. The actual script is
/// generated dynamically at install time (see `message_rules::generate_script`).
/// This placeholder is only used for registration in `ALL` and for health-check
/// matching — the installed script always starts with the same marker comment,
/// so `detect_builtin` identifies it by marker, not by exact content match.
const MESSAGE_RULES_PLACEHOLDER: &str = r#"#!/bin/sh
# gitkit-builtin: message-rules
# This is a placeholder. The real script is generated by gitkit from rules
# configured in .gitmessage-rules.json.
exit 0
"#;
...
mod tests {
    ...
    #[test]
    fn shell_exemption_macro_covers_every_shared_prefix() {
        let exemption = auto_generated_message_exemption!();
        for prefix in AUTO_GENERATED_MESSAGE_PREFIXES {
            assert!(
                exemption.contains(prefix),
                "the shell exemption macro must contain every prefix from \
                 AUTO_GENERATED_MESSAGE_PREFIXES — missing '{prefix}'"
            );
        }
    }

    #[test]
    fn is_auto_generated_message_matches_the_shared_prefixes() {
        for prefix in AUTO_GENERATED_MESSAGE_PREFIXES {
            let subject = format!("{prefix}something");
            assert!(
                is_auto_generated_message(&subject),
                "is_auto_generated_message must return true for prefix '{prefix}'"
            );
        }
        assert!(!is_auto_generated_message("feat: hand-authored"));
        assert!(!is_auto_generated_message("fix: also hand-authored"));
    }
